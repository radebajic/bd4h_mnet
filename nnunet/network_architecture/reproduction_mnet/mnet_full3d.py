"""
MNet with Full 3D VMamba support.

This variant uses CB3dFull3DVMamba which flattens the entire 3D volume (D×H×W)
into a sequence and applies selective scan, providing full volumetric context.

Key differences from mnet.py:
- Uses CB3dFull3DVMamba instead of CBzMamba (Z-axis only)
- Sequence length L = D × H × W (vs L = D for axial)
- Higher memory usage but captures full 3D dependencies
"""
import json
import os
...
from nnunet.network_architecture.reproduction_mnet.basic_module_full3d import (
    CB3d, CB3dSeparable, BasicNet, CB3dFull3DVMamba, VMAMBA_3D_AVAILABLE
)
from typing import Any, Dict, Iterable, List, Optional, Set


# ------------------------
# Utilities / helpers (copied from mnet.py for consistency)
# ------------------------
...
class ChannelGate(nn.Module):
    ...
    def forward(self, x2d: torch.Tensor, x3d: torch.Tensor) -> torch.Tensor:
        ...
        g = torch.sigmoid(self.fc2(self.act(self.fc1(z))))
        return g
...
# ------------------------
# Core blocks (with Full3DVMamba option)
# ------------------------

class Down(BasicNet):
    """
    Down block with Full 3D VMamba support.
    When full3d_vmamba=True, uses CB3dFull3DVMamba instead of CBzMamba.
    """
    def __init__(self, in_channels, out_channels, mode: tuple, FMU='sub', downsample=True, min_z=8,
                 ...
                 full3d_vmamba: bool = False, vmamba_hidden_ratio: float = 1.0, vmamba_d_state: int = 16,
                 vmamba_dropout: float = 0.0, vmamba_use_se: bool = True, vmamba_se_reduction: int = 8):
        
        super().__init__()
        use_vmamba = bool(full3d_vmamba)
        ...
        if self.mode_out in ('3d', 'both'):
            if use_vmamba:
                self.CB3d = CB3dFull3DVMamba(
                    in_channels=in_channels, 
                    out_channels=out_channels,
                    hidden_ratio=vmamba_hidden_ratio,
                    d_state=vmamba_d_state,
                    dropout=vmamba_dropout,
                    use_se=vmamba_use_se,
                    se_reduction=vmamba_se_reduction,
...
class Up(BasicNet):
    """Up block with Full 3D VMamba support."""
    def __init__(self, in_channels, out_channels, mode: tuple, FMU='sub',
        ...
        if gmode is not None:
            assert fuse_ch is not None and fuse_ch_up is not None
    ...
    def forward(self, x):
        ...
        if self.fuse_skip is not None and self.fuse_up is not None:
            fused_skips = self.fuse_skip(xskip2d, xskip3d)
            fused_ups   = self.fuse_up(up2d, up3d)
        else:
...
class MNetFull3D(SegmentationNetwork):
    """
    MNet with Full 3D VMamba support.
    
    When vmamba is enabled for a stage, uses CB3dFull3DVMamba which:
    - Flattens entire 3D volume to sequence L = D × H × W
    - Applies selective scan in 8 directions
    - Captures full volumetric dependencies (vs Z-axis only in original)
    
    Trade-off: Higher memory/compute but richer 3D context.
    """
    # nnU-Net bookkeeping
    ...
    def __init__(
        ...
        # Full 3D VMamba controls
        vm_down_stages: Optional[Iterable[int]] = None,
        vm_up_stages: Optional[Iterable[int]] = None,
        vm_bottleneck_stages: Optional[Iterable[int]] = None,
        vmamba_hidden_ratio: float = 1.0,
        vmamba_d_state: int = 16,
        vmamba_dropout: float = 0.0,
        vmamba_use_se: bool = True,
        vmamba_se_reduction: int = 8,
    ):
        """
        Args:
            vm_down_stages: Encoder stages (1..4) to use Full3DVMamba
            vm_up_stages: Decoder stages (1..4) to use Full3DVMamba
            vm_bottleneck_stages: Bottleneck stages (1..5) to use Full3DVMamba
            vmamba_hidden_ratio: Hidden dimension ratio for VMamba (default: 1.0)
            vmamba_d_state: SSM state dimension (default: 16)
            vmamba_dropout: Dropout rate (default: 0.0)
            vmamba_use_se: Use SE attention (default: True)
            vmamba_se_reduction: SE reduction ratio (default: 8)
        """
        super().__init__()
        ...
        # VMamba kwargs
        vm_kwargs = {
            'vmamba_hidden_ratio': vmamba_hidden_ratio,
            'vmamba_d_state': vmamba_d_state,
            'vmamba_dropout': vmamba_dropout,
            'vmamba_use_se': vmamba_use_se,
            'vmamba_se_reduction': vmamba_se_reduction,
        }
        ...
        self.down11 = Down(in_channels, kn[0], ('/', 'both'), downsample=False,
                           ...
                           full3d_vmamba=(s in vm_down), **vm_kwargs)
        ...
        self.bottleneck1 = Down(kn[3], kn[4], ('2d', '2d'),
                                use_sep3d=use_sep3d, use_checkpoint=use_checkpoint, gated_fusion=gated_fusion,
                                full3d_vmamba=(1 in vm_bn), **vm_kwargs)
        ...
        self.up11 = Up(fct * (kn[3] + kn[4]), kn[3], ('both', '2d'), FMU,
                       ...
                       full3d_vmamba=(s in vm_up), **vm_kwargs)
        ...
        self.bottleneck2 = Down(fct * kn[3], kn[4], ('both', 'both'), FMU,
                                use_sep3d=use_sep3d, use_checkpoint=use_checkpoint, gated_fusion=gated_fusion,
                                full3d_vmamba=(2 in vm_bn), **vm_kwargs)
        ...
        self.bottleneck3 = Down(fct * kn[3], kn[4], ('both', 'both'), FMU,
                                use_sep3d=use_sep3d, use_checkpoint=use_checkpoint, gated_fusion=gated_fusion,
                                full3d_vmamba=(3 in vm_bn), **vm_kwargs)
        ...
        self.bottleneck4 = Down(fct * kn[3], kn[4], ('both', 'both'), FMU,
                                use_sep3d=use_sep3d, use_checkpoint=use_checkpoint, gated_fusion=gated_fusion,
                                full3d_vmamba=(4 in vm_bn), **vm_kwargs)
        ...
        self.bottleneck5 = Down(kn[3], kn[4], ('3d', '3d'),
                                use_sep3d=use_sep3d, use_checkpoint=use_checkpoint, gated_fusion=gated_fusion,
                                full3d_vmamba=(5 in vm_bn), **vm_kwargs)
    ...
    def forward(self, x: torch.Tensor):
        # Forward pass (same as original MNet)
    ...
    def _describe_cb3d_block(block: Optional[nn.Module]) -> Dict[str, Any]:
        ...
        if isinstance(block, CB3dFull3DVMamba):
            entry["impl"] = "CB3dFull3DVMamba"
            entry["vmamba_enabled"] = True
            entry["backend"] = "full3d" if block.vmamba_available else "fallback_conv"
    ...
    def _build_arch_summary(self) -> Dict[str, Any]:
        specs: List[tuple[str, str, int]] = [
            ("down11", "encoder", 1), ("down12", "encoder", 1), ("down13", "encoder", 1), ("down14", "encoder", 1),
            ("bottleneck1", "bottleneck", 1),
            ("down21", "encoder", 2), ("down22", "encoder", 2), ("down23", "encoder", 2),
            ("bottleneck2", "bottleneck", 2),
            ("down31", "encoder", 3), ("down32", "encoder", 3),
            ("bottleneck3", "bottleneck", 3),
            ("down41", "encoder", 4),
            ("bottleneck4", "bottleneck", 4),
            ("bottleneck5", "bottleneck", 5),
            ("up41", "decoder", 4),
            ("up31", "decoder", 3), ("up32", "decoder", 3),
            ("up21", "decoder", 2), ("up22", "decoder", 2), ("up23", "decoder", 2),
            ("up11", "decoder", 1), ("up12", "decoder", 1), ("up13", "decoder", 1), ("up14", "decoder", 1),
        ]
        ...
        return {
            "vmamba_backend_available": VMAMBA_3D_AVAILABLE,
    ...
    def _print_arch_summary(self) -> None:
        ...
        print(f"[Arch] VMamba Full3D backend: {backend_msg}")
        for entry in summary.get("stages", []):
            impl = entry.get("impl")
            backend = entry.get("backend")
            msg = f"[Arch] {entry.get('group')} stage {entry.get('stage')} ({entry.get('name')}): {impl}"
            if backend:
                msg += f" [{backend}]"
            print(msg)
...
if __name__ == '__main__':
    net = MNetFull3D(
        1, 3, kn=(16, 24, 32, 40, 48), ds=True, FMU='sub',
        width_mult=1.0, use_sep3d=False, use_checkpoint=False, cat_reduce=False,
        vm_bottleneck_stages=[4, 5], vmamba_hidden_ratio=1.0, vmamba_d_state=16
    )
